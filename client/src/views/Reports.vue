      ...
      <div class="card">
        ...
        <div class="table-container">
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="row in monthlyWithChanges" :key="row.month">
                ...
                <td>
                  <span v-if="row.prevRevenue !== null" :class="getChangeClass(row.revenue, row.prevRevenue)">
                    {{ getChangeValue(row.revenue, row.prevRevenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
                <td>
                  <span v-if="row.prevRevenue !== null" :class="getChangeClass(row.revenue, row.prevRevenue)">
                    {{ getGrowthRate(row.revenue, row.prevRevenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
              </tr>
...
export default {
  ...
  setup() {
    ...
    // Computed: max revenue so bar height is divided once, not per bar
    const maxRevenue = computed(() => {
      if (monthlyData.value.length === 0) return 1
      return Math.max(...monthlyData.value.map(m => m.revenue))
    })

    // Computed: summary stats derived from loaded data (not imperative writes)
    ...
    // Computed: precompute previous-row revenue so the template doesn't index back into the array
    const monthlyWithChanges = computed(() =>
      monthlyData.value.map((month, index) => ({
        ...month,
        prevRevenue: index > 0 ? monthlyData.value[index - 1].revenue : null
      }))
    )
    ...
    // Bar height uses the computed maxRevenue — no loop per bar
    const barHeight = (revenue) => {
      if (maxRevenue.value === 0) return 0
      return (revenue / maxRevenue.value) * 200
    }

    // Parse YYYY-MM as a local date (append -01T00:00:00 to avoid UTC offset shifts)
    ...
    // Reload when any global filter changes — matches Orders.vue pattern
